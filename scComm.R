...
MakeAugmentedData <- function(expr, scCommRes, lr_database = "./data/LRpairs.txt", prop = 0.05){
    ...
    for (i in 1:nrow(Fulldata)) {
        thispair <- rownames(Fulldata)[i]
        thispair_split <- str_split(thispair, '---')[[1]]
        thisfrom <- thispair_split[1]
        thisto <- thispair_split[2]
        Fulldata[i, ] <- scCommRes$ccires$lrscore[thisfrom, thisto, colnames(Fulldata)]
    }
    rs = rowSums(Fulldata)
    highcut = quantile(rs, 1 - prop)
    ...
    cellname = scCommRes$cellanno$cellname
    progress <- progress_bar$new(total = length(lr_network$from), clear = F)
    rownames(cellanno) = cellanno$cellname
    for (l in 1:length(lr_network$from)) {
        progress$tick()
        SampledSender = sample(cellname, 50)
        SampledReceptor = sample(cellname, 50)
        Senderanno = cellanno[SampledSender, 2]
        Receptoranno = cellanno[SampledReceptor, 2]
        thissd <- sqrt(sd(expr[lr_network$from[l], SampledSender]) * sd(expr[lr_network$to[l], SampledReceptor]))
        if (thissd < 0.1) {
            next()
        }
        exprlij <- sqrt(expr[lr_network$from[l], SampledSender] %*% t(expr[lr_network$to[l], SampledReceptor]))
        exprlij_nonzero <- exprlij[exprlij > 0]
        nonzeroprop <- length(exprlij_nonzero) / length(exprlij)
        ...
        exprlij[exprlij < 0] <- 0
        exprlij[exprlij > 5] <- 5
        cciaddmat <- exprlij * totalweight[Senderanno, Receptoranno, l]
